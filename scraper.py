...
#for site in job_sites:
    ...
    jobs = []
    for job in soup.select("div.my-masonry-grid div.relative"):
        jobs.append({
            'job_title': job.select_one('span.line-clamp-2').text,
            'company': job.select_one('span.line-clamp-1').text,
            'url': job.select_one('a.text-black::attr(href)').attrs.get("href"),
